...
import { ChevronDown, Check, Settings, FileEdit } from "lucide-react";
    ...
    <nav className="bg-[#090909] py-4 px-6 lg:px-8 border-b border-[#202020]">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <div className="hidden md:flex space-x-8">
          <Link href="/#features" className="text-[#828288] hover:text-[#EDECF8] transition-colors text-sm font-medium">
            {t.navigation.features}
          </Link>
          <Link
            href="/#beta_benefits"
            className="text-[#828288] hover:text-[#EDECF8] transition-colors text-sm font-medium"
          >
            {t.navigation.betaBenefits}
          </Link>
          <Link href="/#about" className="text-[#828288] hover:text-[#EDECF8] transition-colors text-sm font-medium">
            {t.navigation.aboutUs}
          </Link>
          <Link href="/#faq" className="text-[#828288] hover:text-[#EDECF8] transition-colors text-sm font-medium">
            {t.navigation.faq}
          </Link>
          <Link href="/menu-dynamic" className="text-[#D78E59] hover:text-[#FFAA6C] transition-colors text-sm font-medium">
            Pages Menu
          </Link>
        </div>
        ...
        <div className="flex items-center gap-4">
          {user && (
            <Link href="/page-manager" className="flex items-center gap-2 px-3 py-2 text-[#828288] hover:text-[#EDECF8] hover:bg-[#202020] rounded-lg transition-colors">
              <Settings className="w-4 h-4" />
              <span className="text-sm font-medium hidden sm:inline">{t.navigation.manager}</span>
            </Link>
          )}
          {user && (
            <Link href="/page-editor" className="flex items-center gap-2 px-3 py-2 text-[#828288] hover:text-[#EDECF8] hover:bg-[#202020] rounded-lg transition-colors">
              <FileEdit className="w-4 h-4" />
              <span className="text-sm font-medium hidden sm:inline">{t.navigation.editor}</span>
            </Link>
          )}
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              className="flex items-center gap-2 px-3 py-2 text-[#828288] hover:text-[#EDECF8] transition-colors rounded-lg hover:bg-[#202020]"
            ...
            {isLanguageOpen && (
              <div className="absolute right-0 top-full mt-2 w-48 bg-[#202020] border border-[#575757] rounded-lg shadow-xl z-50 overflow-hidden">
                {availableLanguages.map((language, index) => (
                  <button
                    key={language.code}
                    className={`w-full text-left px-4 py-3 text-sm flex items-center gap-3 hover:bg-[#171717] transition-colors ${
                      index === 0 ? "rounded-t-lg" : ""
                    } ${index === availableLanguages.length - 1 ? "rounded-b-lg" : ""} ${
                      currentLanguage.code === language.code ? "text-[#D78E59] bg-[#171717]" : "text-[#EDECF8]"
                    }`}
                    type="button"
                  ...
                  >
                    ...
                    {currentLanguage.code === language.code && <Check className="ml-auto w-4 h-4 text-[#D78E59]" />}
                  </button>
                ))}
              </div>
            )}
